...
from handlers import MNIST_Handler, SVHN_Handler, CIFAR10_Handler, openml_Handler, MNIST_Handler_joint, \
    SVHN_Handler_joint, CIFAR10_Handler_joint, ShanghaitechHandler, ShanghaitechHandler_Semi, \
    ShanghaitechHandler_joint, JHUCrowdHandler, SGANetHandler
from data import get_MNIST, get_FashionMNIST, get_EMNIST, get_SVHN, get_CIFAR10, get_CIFAR10_imb, get_CIFAR100, \
    get_TinyImageNet, get_openml, get_BreakHis, get_PneumoniaMNIST, get_waterbirds, get_shanghaitech, get_JHU_Crowd, \
    get_ucf_qnrf, get_ioc_fish
from nets import Net, MNIST_Net, CIFAR10_Net, openml_Net, PneumoniaMNIST_Net, waterbirds_Net, \
    get_net_vae
from nets_crowdcount import CSR_Net, CSR_Net_PSSW, Net_Crowd_Counting_Semi, CSR_Net_Semi, \
    Net_Crowd_Counting_IRAST, CSR_Net_IRAST, Net_Crowd_Counting_Project, CSR_Net_Project, Net_Crowd_Counting_SGANet, \
    SGANet, CANNet
from nets_crowd_project import Net_Crowd_Counting_Project_DUBNet, DUBNet, CSR_DUBNet
from nets_lossprediction import Net_LPL, Net_LPL_CSR, MNIST_Net_LPL, CIFAR10_Net_LPL, PneumoniaMNIST_Net_LPL, \
    waterbirds_Net_LPL, \
    CSR_Net_LPL, get_lossnet
...
from query_strategies import RandomSampling, LeastConfidence, MarginSampling, EntropySampling, \
    ...
    LossPredictionLoss, VAAL, WAAL, ConsistencyCrowd, CrowdRank, HeadCount, TA_VAAL, ActiveSegCrowd
from initial_sampler import RandomSampler, AutoEncoderSampler
...
def get_handler(name):
    if name == 'MNIST':
        ...
        return CIFAR10_Handler
    elif name == 'shanghaitechA' or name == 'shanghaitechB':
        return ShanghaitechHandler
    elif name == 'JHUCrowd' or name == 'UCF-QNRF' or name == 'IOCfish':
        return JHUCrowdHandler
    else:
...
def get_handler_joint(name):
    if name == 'MNIST':
        ...
        return ShanghaitechHandler_joint
    else:
        raise NotImplementedError
...
def get_dataset(name, args_task):
    if name == 'MNIST':
        ...
        return get_waterbirds(get_handler(name), args_task)
    elif name == 'shanghaitechA':
        return get_shanghaitech(get_handler(name), args_task, 'A')
    elif name == 'shanghaitechB':
        return get_shanghaitech(get_handler(name), args_task, 'B')
    elif name == 'JHUCrowd':
        return get_JHU_Crowd(get_handler(name), args_task)
    elif name == 'UCF-QNRF':
        return get_ucf_qnrf(get_handler(name), args_task)
    elif name == 'IOCfish':
        return get_ioc_fish(get_handler(name), args_task)
    else:
        raise NotImplementedError


def get_dataset_sganet(name, args_task):
    if name == 'shanghaitechA':
        return get_shanghaitech(SGANetHandler, args_task, 'A')
    elif name == 'shanghaitechB':
        return get_shanghaitech(SGANetHandler, args_task, 'B')
    elif name == 'JHUCrowd':
        return get_JHU_Crowd(SGANetHandler, args_task)
    elif name == 'UCF-QNRF':
        return get_ucf_qnrf(SGANetHandler, args_task)
    else:
        raise NotImplementedError
...
def get_net(name, args_task, device, net=None):
    if name == 'MNIST':
        ...
        if net == 'sganet':
            return Net_Crowd_Counting_SGANet(SGANet, args_task, device)
        return Net_Crowd_Counting_Project(CSR_Net_Project, args_task, device)
    elif name == 'JHUCrowd':
        return Net_Crowd_Counting_Project(CSR_Net_Project, args_task, device)
    elif name == 'UCF-QNRF' or name == 'IOCfish':
        return Net_Crowd_Counting_Project(CSR_Net_Project, args_task, device)
    else:
        raise NotImplementedError
...
# TODO 这里改了
def get_net_pssw(name, args_task, device):
    return Net_Crowd_Counting_Project(CSR_Net_Project, args_task, device)


def get_net_crowd_semi(name, args_task, device):
    return Net_Crowd_Counting_Semi(CSR_Net_Semi, args_task, device)


def get_net_crowd_irast(name, args_task, device):
    return Net_Crowd_Counting_IRAST(CSR_Net_IRAST, args_task, device)


# def get_net_crowd_project(name, args_task, device, net):
#     if net == 'sganet':
#         return Net_Crowd_Counting_SGANet(SGANet, args_task, device)
#     if net == 'cannet':
#         return Net_Crowd_Counting_CANNet(CANNet, args_task, device)
#     return Net_Crowd_Counting_Project(CSR_Net_Project, args_task, device)


def get_net_lpl(name, args_task, device, strategy):
    ...
    if name == 'MNIST':
        ...
        return Net_LPL_CSR(CSR_Net_LPL, args_task, device, loss_net, TA_VAAL=strategy == 'TA-VAAL')
    else:
        raise NotImplementedError
...
def get_net_waal(name, args_task, device):
    ...
    if name == 'MNIST':
        ...
        return Net_WAAL(waterbirds_Net_WAAL, args_task, device, waterbirds_Net_WAAL, CLF_WAAL, Discriminator,
                        handler_joint)
    elif name == 'waterbirds_pretrain':
        return Net_WAAL(waterbirds_Net_WAAL, args_task, device, waterbirds_Net_WAAL, CLF_WAAL, Discriminator,
                        handler_joint)
...
def get_strategy(STRATEGY_NAME, dataset, net, args_input, args_task):
    ...
    elif STRATEGY_NAME == 'ConsistencyCrowd':
        return ConsistencyCrowd(dataset, net, args_input, args_task)
    elif STRATEGY_NAME == 'CrowdRank':
        return CrowdRank(dataset, net, args_input, args_task)
    elif STRATEGY_NAME == 'HeadCount':
        return HeadCount(dataset, net, args_input, args_task)
    elif STRATEGY_NAME == 'TA-VAAL':
        net_vae, net_disc = get_net_vae(args_task['name'])
        handler_joint = get_handler_joint(args_task['name'])
        return TA_VAAL(dataset, net, args_input, args_task, net_vae=net_vae, net_dis=net_disc,
                       handler_joint=handler_joint)
    elif STRATEGY_NAME == 'ActiveSegCrowd':
        return ActiveSegCrowd(dataset, net, args_input, args_task)
    else:
        raise NotImplementedError


def get_sampler(SAMPLER_NAME, dataset, args_input, args_task):
    if SAMPLER_NAME == 'RandomSampler':
        return RandomSampler(dataset, args_input, args_task)
    elif SAMPLER_NAME == 'AESampler':
        return AutoEncoderSampler(dataset, args_input, args_task)
    else:
        raise NotImplementedError
