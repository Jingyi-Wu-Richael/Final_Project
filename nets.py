...
class Net:
    ...
    def train(self, data, test_data=None):
        ...
        if self.params['optimizer'] == 'Adam':
            ...
            lr_schedule = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[160], gamma=0.1)
        else:
...
class VAE_ImageNet(nn.Module):
    ...
    def __init__(self, z_dim=64, nc=3, TA_VAAL=False):
        super(VAE_ImageNet, self).__init__()
        self.z_dim = z_dim
        self.nc = nc
        self.encoder = nn.Sequential(
            nn.Conv2d(nc, 64, 4, 2, 1, bias=False),              # B,  64, 112, 112
            nn.BatchNorm2d(64),
            nn.ReLU(True),
            ...
            nn.BatchNorm2d(1024),
            nn.ReLU(True),
            View((-1, 1024 * 7 * 7)),                                 # B, 1024*4*4
        )

        self.fc_mu = nn.Linear(1024 * 7 * 7, z_dim)                            # B, z_dim
        self.fc_logvar = nn.Linear(1024 * 7 * 7, z_dim)                            # B, z_dim

        # latent dim is +1 for TA-VAAL
        z_dim_decoder = z_dim + 1 if TA_VAAL else z_dim

        self.decoder = nn.Sequential(
            nn.Linear(z_dim_decoder, 1024 * 7 * 7),                           # B, 1024*7*7
            ...
            nn.BatchNorm2d(64),
            nn.ReLU(True),
            nn.ConvTranspose2d(64, 32, 4, 2, 1, bias=False),      # B,   32, 224, 224
            nn.BatchNorm2d(32),
            nn.ReLU(True),
            nn.ConvTranspose2d(32, nc, 1),                       # B,    nc, 224, 224
        )
        self.weight_init()
