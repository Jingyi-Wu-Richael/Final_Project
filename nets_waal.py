...
class Net_WAAL:
    ...
    def train(self, data, X_labeled, Y_labeled, X_unlabeled, Y_unlabeled, alpha=1e-3):
        ...
        dim = data.X.shape[1:]
        self.fea = self.net_fea(dim=dim, pretrained=self.params['pretrained'], num_classes=self.params['num_class']).to(
            self.device)
        self.clf = self.net_clf(dim=dim, pretrained=self.params['pretrained'], num_classes=self.params['num_class']).to(
            self.device)
        ...
        for epoch in tqdm(range(1, n_epoch + 1), ncols=100):
            ...
            for index, label_x, label_y, unlabel_x, _ in loader_tr:
                ...
                # for CIFAR10_adam the gradient penality is 5
