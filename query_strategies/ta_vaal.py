...
class TA_VAAL(Strategy):
    def __init__(self, dataset, net, args_input, args_task, net_vae, net_dis, handler_joint):
        super(TA_VAAL, self).__init__(dataset, net, args_input, args_task)
    ...
    def query(self, n):
        unlabeled_idxs, unlabeled_data = self.dataset.get_unlabeled_data_vaal()
        self.train_vaal()
        uncertainties = self.pred_dis_score_vaal(unlabeled_data)
        return unlabeled_idxs[uncertainties.sort(descending=True)[1][:n]]

    def train_vaal(self, total_epoch=100, num_vae_steps=1, beta=1, adv_param=1, latent_dim=64, batch_size=10):
        ...
        # VAE module
        self.vae = self.net_vae(latent_dim, TA_VAAL=True).to(self.device)
        # Discriminator module
        self.dis = self.net_dis(latent_dim + 1).to(self.device)
        ...
        for epoch in tqdm(range(n_epoch)):
            ...
            for index, label_x, label_y, unlabel_x, _ in loader_tr:
                ...
                if iteration_count == 0:
                    r_l_0 = torch.from_numpy(np.random.uniform(0, 1, size=(label_x.shape[0], 1))).type(
                        torch.FloatTensor).to(self.device)
                    r_u_0 = torch.from_numpy(np.random.uniform(0, 1, size=(unlabel_x.shape[0], 1))).type(
                        torch.FloatTensor).to(self.device)
                else:
                    with torch.no_grad():
                        _, features_l = self.net(label_x)
                        _, feature_u = self.net(unlabel_x)
                        r_l = self.net.net_lpl(features_l)
                        r_u = self.net.net_lpl(feature_u)
                if iteration_count == 0:
                    r_l = r_l_0.detach()
                    r_u = r_u_0.detach()
                    r_l_s = r_l_0.detach()
                ...
                for count in range(num_vae_steps):
                    recon, z, mu, logvar = self.vae(label_x, r_l_s)
                    unsup_loss = vae_loss(label_x, recon, mu, logvar, beta)
                    unlabel_recon, unlabel_z, unlabel_mu, unlabel_logvar = self.vae(unlabel_x, r_u_s)
                    transductive_loss = vae_loss(unlabel_x, unlabel_recon, unlabel_mu, unlabel_logvar, beta)
                    label_preds = self.dis(mu, r_l_s)
                    unlabel_preds = self.dis(unlabel_mu, r_u_s)

                    label_preds_real = torch.ones(label_x.size(0)).to(self.device)
                    unlabel_preds_real = torch.ones(unlabel_x.size(0)).to(self.device)
                    bce_loss = nn.BCELoss()
                ...
                for count in range(num_vae_steps):
                    with torch.no_grad():
                        _, _, mu, _ = self.vae(label_x, r_l_s)
                        _, _, unlabel_mu, _ = self.vae(unlabel_x, r_u_s)
    ...
    def pred_dis_score_vaal(self, data):
        ...
        with torch.no_grad():
            for x, y, idxs in loader_te:
                ...
                _, features = self.net.clf(x)
                r = self.net.net_lpl(features)

                _, _, mu, _ = self.vae(x, torch.sigmoid(r))
                out = self.dis(mu, r).cpu()
